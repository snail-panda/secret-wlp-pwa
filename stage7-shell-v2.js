(() => {
  ...
  // v1.8.6.3: the shared shell is loaded directly by each target page.
  // Load its CSS from the same root so shell delivery no longer depends on
  // pwa-register.js or service-worker timing.
  const ensureShellStyle = () => {
    if (document.querySelector('link[data-stage7-shell-v2]')) return;
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = rootUrl('stage7-shell-v2.css?v=20260917-s7-shell-v1863');
    link.dataset.stage7ShellV2 = 'true';
    document.head.appendChild(link);
  };
  ensureShellStyle();
  document.documentElement.dataset.wlpShellVersion = '1.8.6.3';
