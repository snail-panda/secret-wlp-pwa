...
function fillFixedFrame(frame, slot) {
  const source = clean(frame);
  const value = clean(slot);
  if (!source || !value || (source.match(/\{answer\}/g) || []).length !== 1) return '';
  return clean(source.replace('{answer}', value));
}

function recoverVerifiedFixedFrameNaturalOptions(normalized, requestContext = {}) {
  const learner = obj(normalized?.learnerFacingResponse);
  if (Array.isArray(learner.naturalOptions) && learner.naturalOptions.map(clean).filter(Boolean).length) return normalized;
  const experience = obj(requestContext?.experience);
  if (clean(experience.responseConstraint).toLowerCase() !== 'fixed-frame' || experience.frameCompatibilityVerified !== true) return normalized;
  const frame = clean(experience.responseFrame);
  if ((frame.match(/\{answer\}/g) || []).length !== 1) return normalized;
  const learnerAnswer = clean(requestContext?.learnerResponse?.authoritativeResponse);
  const intended = clean(experience.intendedExample);
  const revision = clean(learner.suggestedNaturalForm);
  const blockedFull = new Set([intended, revision].map(feedbackKey).filter(Boolean));
  const blockedSlots = new Set([learnerAnswer].map(feedbackKey).filter(Boolean));
  const recovered = [];
  for (const slot of arr(experience.frameCompatibleAlternatives)) {
    const slotText = clean(slot);
    if (!slotText || blockedSlots.has(feedbackKey(slotText))) continue;
    const full = fillFixedFrame(frame, slotText);
    const key = feedbackKey(full);
    if (!key || blockedFull.has(key) || recovered.some(item => feedbackKey(item) === key)) continue;
    recovered.push(full);
    if (recovered.length >= 3) break;
  }
  if (!recovered.length) return normalized;
  learner.naturalOptions = recovered;
  normalized.learnerFacingResponse = learner;
  return normalized;
}

function normalizeInterpreterContract(result, requestContext = {}) {
  let normalized = clone(result) || {};
  ...
  const learner = obj(normalized.learnerFacingResponse);
  if (Array.isArray(learner.naturalOptions)) {
    const seenNaturalOptions = new Set();
    learner.naturalOptions = learner.naturalOptions.map(item => clean(item)).filter(item => {
      const key = item.normalize('NFKC').toLowerCase().replace(/\s+/g, ' ');
      if (!key || seenNaturalOptions.has(key)) return false;
      seenNaturalOptions.add(key);
      return true;
    }).slice(0, 3);
    normalized.learnerFacingResponse = learner;
  }
  ...
  normalized = recoverVerifiedFixedFrameNaturalOptions(normalized, requestContext);
  return normalized;
}

function finalizeProviderOutput(kind, output, requestContext = {}) {
  let result = clone(output?.result) || {};
  if (kind === 'planner') result = normalizePlannerExperienceContract(result);
  if (kind === 'interpreter') result = normalizeInterpreterContract(result, requestContext);
...
async function callProvider(kind, payload, eventId) {
  ...
  return finalizeProviderOutput(kind, output, minimizedPayload);
}
...
exports._test = Object.freeze({
  ...
  normalizeInterpreterContract,
  recoverVerifiedFixedFrameNaturalOptions,
  fillFixedFrame
});
